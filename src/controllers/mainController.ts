...
export default class MainController implements vscode.Disposable {
    ...
    /**
     * Choose a language flavor for the SQL document. Should be either "PGSQL" or "Other"
    ...
    private showReleaseNotesPrompt(): void {
        ...
        if (!this.doesExtensionLaunchedFileExist()) {
            ...
            this._vscodeWrapper.showInformationMessage(
                    'View pgsql for Visual Studio Code release notes?', confirmText)
                .then((choice) => {
    ...
    public onDidOpenTextDocument(doc: vscode.TextDocument): void {
        ...
        if (doc && doc.languageId === Constants.languageId) {
            this._statusview.languageFlavorChanged(doc.uri.toString(), Constants.pgsqlProviderName);
        }
